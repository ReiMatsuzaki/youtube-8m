...
class FrameLevelTopkMoeModel(models.BaseModel):
  def create_model(self, model_input, vocab_size, num_frames,
                   l2_penalty=1e-4, is_training=True, **unused):
    ...
    logging.info('model_input : {0}'.format(model_input.get_shape().as_list() ))
    logging.info('xt : {0}'.format(xt.get_shape().as_list() ))
    logging.info('tk : {0}'.format(tk.get_shape().as_list() ))
    logging.info('topk : {0}'.format(topk.get_shape().as_list() ))
    logging.info('inter_f_mean : {0}'.format(inter_f_mean.get_shape().as_list() ))
    logging.info('inter_f_std : {0}'.format(inter_f_std.get_shape().as_list() ))

    inter_f_feats = tf.concat([inter_f_mean, inter_f_std, topk], 1)

    logging.info('inter_f_feats : {0}'.format(inter_f_feats.get_shape().as_list() ))
